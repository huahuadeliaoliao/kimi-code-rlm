...
describe('Plan service', () => {
  ...
  beforeEach(() => {
    ...
    profile.addActiveTool('EnterPlanMode');
    profile.addActiveTool('ExitPlanMode');
  });
  ...
  it('rejects new plan state and hides restored reminders when plan tools are inactive', async () => {
    await plan.enter('historical-plan', false);
    await injector.inject(undefined, true);
    expect(
      context
        .get()
        .some(
          (message) =>
            message.origin?.kind === 'injection' && message.origin.variant === 'plan_mode',
        ),
    ).toBe(true);
    context.append({
      role: 'user',
      content: [{ type: 'text', text: 'HISTORICAL_SWARM_REMINDER' }],
      toolCalls: [],
      origin: { kind: 'injection', variant: 'swarm_mode' },
    });
    profile.update({ activeToolNames: ['RlmKernel'] });

    await injector.inject(undefined, true);
    ctx.mockNextResponse({ type: 'text', text: 'No historical mode guidance.' });
    await ctx.rpc.prompt({ input: [{ type: 'text', text: 'Continue normally' }] });
    await ctx.untilTurnEnd();

    expect(JSON.stringify(ctx.llmCalls[0]?.history)).not.toContain('plan mode');
    expect(JSON.stringify(ctx.llmCalls[0]?.history)).not.toContain('HISTORICAL_SWARM_REMINDER');
    await expect(plan.enter('new-plan', false)).rejects.toThrow(
  ...
  function useTools(tools: readonly string[]): void {
    profile.update({ activeToolNames: ['EnterPlanMode', 'ExitPlanMode', ...tools] });
    ctx.newEvents();
  }
  ...
  describe('plan allows safe tool flow', () => {
    ...
    it('allows read-only Bash to continue through permission and execution', async () => {
      ...
      expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
        ...
        [wire] llm.request                 { "kind": "loop", "provider": "openai", "model": "mock-model", "modelAlias": "mock-model", "thinkingEffort": "off", "maxTokens": 1000000, "toolSelect": false, "systemPromptHash": "ec9c34379c88babbc468ef2f3e0e08cd2f422c8c4a910664fb8bb394d703a575", "toolsHash": "57c0aaab85782e6df5c4ef33d5a1214f45da5f71887a88f5da1b91c94a8cee0f", "messageCount": 4, "turnStep": "0.2", "time": "<time>" }
